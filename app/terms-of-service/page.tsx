...
export default function TermsOfServicePage() {
  ...
  useEffect(() => {
    fetch('/api/legal')
      .then(r => r.json())
      .then(d => {
        if (d.terms_of_service?.trim()) {
          setContent(d.terms_of_service.trim())
        } else {
          setNotFound(true)
        }
      })
  ...
  return (
    <div className="min-h-screen bg-background text-foreground relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-0 right-1/4 w-[500px] h-[300px] bg-cyan-500/5 rounded-full blur-3xl" />
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-blue-600/4 rounded-full blur-3xl" />
        <div className="absolute top-1/2 right-0 w-[300px] h-[300px] bg-cyan-500/3 rounded-full blur-3xl" />
      </div>
        ...
        <AnimatePresence mode="wait">
          {loading && (
            ...
            >
              ...
              <p className="text-muted-foreground text-sm">Loading Terms of Service…</p>
            </motion.div>
          )}
          ...
          {!loading && notFound && (
            ...
            >
              ...
              <div>
                <h2 className="text-xl font-bold mb-1">Terms of Service Not Available</h2>
          ...
          {!loading && !notFound && (
            ...
            >
              ...
              >
                {/* Top accent bar */}
                <div className="h-1 bg-gradient-to-r from-cyan-500 via-blue-600 to-cyan-500 bg-[length:200%_100%] animate-[shimmer_3s_linear_infinite]" />
                    ...
                    {content.split('\n').map((line, i) => {
                      if (line.startsWith('## ')) {
                        return (
                          ...
                          >
                            <span className="w-1.5 h-5 rounded bg-gradient-to-b from-cyan-500 to-blue-600 inline-block shrink-0" />
                            {line.replace('## ', '')}
                          </motion.h2>
                        )
                      }
